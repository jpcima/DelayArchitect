...
Gd *GdNew(unsigned numinputs, unsigned numoutputs)
{
    ...
    if (numinputs == 1) {
        gd->network_.resize(1);
        gd->network_[0].setChannelMode(GdNetwork::ChannelMode::Mono);
    }
    else if (numinputs == 2) {
        gd->network_.resize(2);
        gd->network_[0].setChannelMode(GdNetwork::ChannelMode::StereoLeft);
        gd->network_[1].setChannelMode(GdNetwork::ChannelMode::StereoRight);
    }
    else {
        return nullptr;
    }
...
void GdClear(Gd *gd)
{
    ...
    for (GdNetwork &network : gd->network_)
        network.clear();
}
...
void GdProcess(Gd *gd, const float *inputs[], float *outputs[], unsigned count)
{
    ...
    ///
    unsigned numinputs = (unsigned)gd->network_.size();

    switch (numinputs) {
    case 1:
        {
            float *mono = gd->temp_[2].data();
            std::copy_n(inputs[0], count, mono);
            std::fill_n(outputs[0], count, 0.0f);
            gd->network_[0].process(mono, dry, wet, outputs, count);
        }
        break;
    case 2:
        {
            float *left = gd->temp_[2].data();
            float *right = gd->temp_[3].data();
            std::copy_n(inputs[0], count, left);
            std::copy_n(inputs[1], count, right);
            std::fill_n(outputs[0], count, 0.0f);
            std::fill_n(outputs[1], count, 0.0f);
            gd->network_[0].process(left, dry, wet, outputs, count);
            gd->network_[1].process(right, dry, wet, outputs, count);
        }
        break;
    default:
        assert(false);
...
void GdSetParameterEx(Gd *gd, GdParameter p, float value, bool force)
{
    ...
    for (GdNetwork &network : gd->network_)
        network.setParameter((unsigned)p, value);
}
